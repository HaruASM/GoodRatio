...
import { createMessage } from '../models/communityModels'; // communityModels 경로 확인 필요
...
const dispatchCustomEvent = (eventName, detail, target = window) => {
  const event = new CustomEvent(`community:${eventName}`, { detail });
  target.dispatchEvent(event);
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[realtimeChatUtilsFB] 이벤트 발행: community:${eventName} on ${target === window ? 'window' : 'DOM element'}`, detail);
  }
};
// firebaseStorage도 chatFirebaseUtils에서 가져옴
...
export const sendMessage = async (roomId, messageData, options = {}) => {
  try {
    ...
    dispatchCustomEvent('messageSent', {
      roomId,
      messageId: docRef.id,
      messageData: newMessage, // newMessage 객체를 사용 (DB에 저장된 형태와 유사)
    }, target);
    ...
    dispatchCustomEvent('error', { roomId, error: error.message }, options.target || window);
 ...
 */
export const setupChatListener = (roomId, communityDBManager, options = {}, target = window) => {
