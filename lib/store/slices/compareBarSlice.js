...
const compareBarSlice = createSlice({
  ...
  reducers: {
    ...
    // 구글 검색 동기화 상태 설정
    setSyncGoogleSearch: (state, action) => {
      const newValue = action.payload !== undefined ? action.payload : true;
      console.log(`setSyncGoogleSearch 호출됨: ${newValue}`);
      state.isSyncGoogleSearchCompareBar = newValue;
    },
