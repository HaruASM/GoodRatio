...
const imageManagerSlice = createSlice({
  ...
  reducers: {
    ...
    openImageGallery: (state, action) => {
      ...
      // 이미지 배열을 직접 전달받아 저장
      if (action.payload.imageList && Array.isArray(action.payload.imageList)) {
        state.viewGalleryImages = action.payload.imageList;
      } else {
        // 이미지 배열이 없으면 빈 배열 설정 (예외 처리 없음)
        state.viewGalleryImages = [];
      }
      
      // source 값은 이미지 확인 갤러리에서 실제로 사용되지 않으므로 제거
      // state.source = action.payload.source || 'rightSidebar';
      
      // 기존 호환성 유지 코드도 불필요하므로 제거
      // if (action.payload.images) {
      //   state.mainImage = action.payload.images.mainImage || '';
      //   state.subImages = action.payload.images.subImages || [];
      // }
    },
    ...
    closeImageGallery: (state) => {
      ...
      state.viewGalleryImages = []; // 갤러리 닫을 때 이미지 배열 초기화
    },
