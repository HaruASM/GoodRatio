...
export function getPublicIdFromGoogleReference(photoReference) {
  ...
  console.log(`생성된 publicId: ${publicId} (물리적 저장 경로: ${ASSET_FOLDER_FOR_PLACE}/${publicId})`);
  return publicId;
}
 ...
 */
export async function uploadGooglePlaceImage(photoReference, maxWidth = 800, apiKey) {
  if (!photoReference) {
    throw new Error('유효한 photo_reference가 필요합니다');
  }
