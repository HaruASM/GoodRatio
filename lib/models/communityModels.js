...
/**
 * 커스텀 이벤트 디스패치 헬퍼 함수
 * @param {string} eventType - CommunityEventTypes의 이벤트 타입
 * @param {Object} detail - 이벤트 데이터 (MessagesUpdatedEvent | TypingStatusEvent | MessageSentEvent | MessageDeletedEvent | RoomMemberChangedEvent | ErrorEvent)
 * @param {EventTarget} [target=window] - 이벤트를 디스패치할 대상 DOM 요소 또는 window
 */
export const dispatchCommunityEvent = (eventType, detail, target = window) => {
  if (!target || typeof target.dispatchEvent !== 'function') {
    console.warn(`[CommunityModels] 유효하지 않은 이벤트 대상: ${eventType}`);
    return;
  }
  const event = new CustomEvent(eventType, { detail });
  target.dispatchEvent(event);
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[CommunityModels] 이벤트 발행: ${eventType} on ${target === window ? 'window' : 'DOM element'}`, detail);
  }
};