  ...
  const unsubscribe = onSnapshot(sectionRef, async (sectionSnapshot) => {
    if (sectionSnapshot.exists()) {
      ...
      if (sectionData.lastUpdated) {
        ...
        console.log(`${sectionName} 로컬 타임스탬프: ${localTimestamp || '없음'}`);
        
        // 타임스탬프 비교: 로컬 데이터가 없거나 서버 데이터가 더 최신인 경우에만 업데이트
        if (!localTimestamp || Number(serverTimestamp) > Number(localTimestamp)) {
          console.log(`${sectionName} 데이터 업데이트 필요 (서버: ${serverTimestamp}, 로컬: ${localTimestamp || '없음'})`);
          ...
          console.log(`${sectionName} 데이터가 이미 최신 상태입니다 (서버: ${serverTimestamp}, 로컬: ${localTimestamp})`);
        }
      }
    }
  });
