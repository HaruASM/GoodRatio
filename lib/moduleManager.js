...
const ModuleManager = {
  ...
  initialize: async function() {
    ...
    moduleState.initializingPromise = new Promise(async (resolve) => {
      ...
      // CommunityDBManager 모듈 등록 (지연 초기화)
      ModuleManager.register('communityDBManager', async () => {
        console.log('[ModuleManager] CommunityDBManager 모듈 동적 로드 시작');
        // 동적으로 CommunityDBManager 가져오기
        const { default: CommunityDBManager } = await import('../lib/components/data/CommunityDBManager');
        console.log('[ModuleManager] CommunityDBManager 모듈 동적 로드 완료');
        // 모듈 인스턴스 반환
