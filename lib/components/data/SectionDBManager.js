 ...
 *    - subscribe() 호출로 구독 시작, 반환된 함수 호출로 구독 해제
 * 
 * 4. 데이터 업데이트 흐름:
 *    - getSectionItems() 호출 -> _setupRealtimeListener() 설정 -> Firebase 업데이트 감지
 *    - 업데이트 필요 확인 -> 데이터 가져오기 -> 캐시 업데이트 -> _notifySubscribers() 호출
 * 
 * 5. 사용 예시:
...
const SectionDBManager = {
  ...
  _setupRealtimeListener: function(sectionName) {
    ...
    // 이후 counterCollections자료구조 값을 조회하여 특정 콜렉션 변경 여부 확인
          ...
          .then(collectionsData => {
            ...
            collectionsData.forEach(collection => {
              ...
              if (collection.nameCollection === 'items' && collection.data.length > 0) {
                ...
                this._notifySubscribers(sectionName, clientItems);
  ...
  _handleSectionUpdate: function(event) {
    ...
    if (shouldUpdate) {
      ...
      // 구독자에게 알림
      this._notifySubscribers(sectionName, items);
   ...
   * @private
   */
  _notifySubscribers: function(sectionName, items) {
   ...
   */
  _notifySubscribers: function(sectionName, items) {
    if (!moduleState._subscribers.has(sectionName)) return;
    
    const subscribers = moduleState._subscribers.get(sectionName);
    if (subscribers.size === 0) return;
    
    console.log(`[SectionDBManager] ${sectionName} 섹션 구독자 ${subscribers.size}명에게 업데이트 알림`);
