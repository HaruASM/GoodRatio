  ...
  initialize: function(mapInstance) {
    ...
    try {
      ...
      if (window.google.maps.marker && window.google.maps.marker.AdvancedMarkerElement) {
        // ICON_DESIGN.HEARTRETRO 아이콘 생성
        const { createIconByiconDesign, ICON_DESIGN } = require('./MapIcons');
        const heartRetroIcon = createIconByiconDesign(ICON_DESIGN.HEARTRETRO);
        
        // 마커 스타일 설정
        heartRetroIcon.style.cursor = 'pointer';
        
        // AdvancedMarkerElement 생성
        const heartRetroMarkerView = new google.maps.marker.AdvancedMarkerElement({
          map: this._mapInstance,
          position: markerPosition,
          content: heartRetroIcon,
          title: "HEARTRETRO 아이콘"
        });
        
        console.log('[DEBUG] HEARTRETRO 아이콘 샘플 마커 생성 완료');
      } else {
        ...
        const fallbackMarker = new google.maps.Marker({
          position: markerPosition,
          map: this._mapInstance,
          title: 'HEARTRETRO 아이콘 (대체 마커)'
        });
      }
    } catch (error) {
      console.error('[ERROR] HEARTRETRO 아이콘 샘플 생성 실패:', error.message);
    }
  },
