...
/**
 * 지도를 특정 위치로 이동
 * @param {Object} center 이동할 중심 좌표 ({ lat, lng })
 * @param {number} zoom 줌 레벨 (선택사항)
 * @param {boolean} animate 애니메이션 적용 여부 (선택사항, 기본값: true)
 * @param {Function} callback 이동 완료 후 실행할 콜백 함수 (선택사항)
 * @returns {Promise<void>} 이동 완료 후 resolve되는 Promise
 */
const panToLocation = (center, zoom = null, animate = true, callback = null) => {
  return new Promise((resolve) => {
    if (!mapInstance) {
      console.error('[GoogleMapManager] 맵 인스턴스가 초기화되지 않았습니다.');
      resolve();
    ...
    if (!center || typeof center.lat !== 'number' || typeof center.lng !== 'number') {
      console.error('[GoogleMapManager] 유효하지 않은 중심 좌표:', center);
      resolve();
      return;
    }
    
    console.log(`[GoogleMapManager] 지도 이동: lat=${center.lat}, lng=${center.lng}, zoom=${zoom}, animate=${animate}`);
