...
const OverlayService = {
  ...
  createImageMarker: async function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    ...
    // 인자로 받은 좌표를 그대로 사용
    console.log('[OverlayService] 좌표 사용:', coordinates);
    ...
    try {
      ...
      if (publicId) {
        try {
          ...
          // publicId 처리는 클라이언트에서 하지 않고 imageHelpers.js와 API에 위임
          const { createDomImgProps, IMAGE_TEMPLATES } = await import('../../utils/imageHelpers');
          ...
          if (imgProps && imgProps.src) {
            ...
            console.log('[OverlayService] 이미지 URL 가져옴:', imageUrl);
          } else {
            imageUrl = fallbackImageUrl;
            console.log('[OverlayService] 이미지 URL을 가져올 수 없음, 폴백 이미지 사용');
          }
        } catch (error) {
        ...
        imageUrl = 'https://images.unsplash.com/photo-1514924013411-cbf25faa35bb?q=80&w=600&auto=format&fit=crop';
        console.log('[OverlayService] 대체 이미지 사용 (publicId 없음)');
      }
      ...
      return new OverlayDelegate(imageMarker, 'imageMarker');
  ...
  createToggleableImageMarker: async function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    // 기본 이미지 마커 생성 (이미 OverlayDelegate로 래핑되어 있음)
    const imageMarkerDelegate = await this.createImageMarker(mapInstance, coordinates, publicId, title, baseWidth, baseHeight, options);
    
    if (!imageMarkerDelegate) return null;
    
    // 마커 초기 스타일 설정 (기본적으로 숨김)
    if (!imageMarkerDelegate.overlay.style) imageMarkerDelegate.overlay.style = {};
    imageMarkerDelegate.overlay.style.display = 'none';
