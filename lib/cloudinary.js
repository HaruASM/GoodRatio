...
const ASSET_FOLDER_FOR_PLACE = 'map-Images';

/**
 * 구글 Place 이미지의 Cloudinary 공개 ID를 생성합니다.
 * 계층 구조: placeImages/섹션/아이템ID/이미지해시
 * 
 * @param {string} photoReference - 구글 Place 포토 레퍼런스
 * @param {string} section - 섹션 이름 (지역명: '반월당', '세부', '앙헬레스', '말라떼' 등) - 이 함수는 구글 레퍼런스 전용이므로 'tempsection' 사용
 * @param {string} itemId - 파이어베이스 서버의 아이템 ID (파이어베이스에 저장된 고유 문서 ID) - 이 함수는 구글 레퍼런스 전용이므로 구글placeID를 사용
 * @returns {string} Cloudinary 공개 ID (폴더 포함)
 */
export function getPublicIdFromReference(photoReference, section, itemId) {
  if (!photoReference) {
    throw new Error('유효한 photo_reference가 필요합니다');
  }
  
  if (!section) {
    throw new Error('유효한 section이 필요합니다');
  }
  
  if (!itemId) {
    throw new Error('유효한 itemId가 필요합니다');
  }
  ...
  // 계층형 구조로 공개 ID 구성: placeImages/지역명/아이템ID/이미지해시
  return `${BASE_FOLDER}/${section}/${itemId}/${imageHash}`;
}
...
export async function uploadGooglePlaceImage(photoReference, maxWidth = 800, apiKey, placeId) {
  ...
  try {
    ...
    // 플레이스 ID 확인
    if (!placeId) {
      throw new Error('구글 이미지 업로드에는 placeId가 필요합니다');
    }

    // (구글이미지 관련 섹션은 'tempsection' 사용)
    const section = 'tempsection'; 
    const provider = 'google_place_api';

    // 계층형 구조의 공개 ID 생성 - 세 개의 필수 매개변수 제공
    const publicId = getPublicIdFromReference(photoReference, section, placeId);
  
    // 구글 API에서 이미지 가져오기 - image-proxy.js와 동일한 방식 적용
    ...
    if (maxWidth !== null) {
      ...
      console.log(`구글 Place 이미지 가져오는 중 (기본 너비: 800px): ${photoReference.substring(0, 10)}...`);
    }
    
    // image-proxy.js와 동일한 방식으로 요청
    const response = await fetch(placesApiUrl);
    
    // 오류 처리 (image-proxy.js와 동일하게)
    if (!response.ok) {
