...
const MapOverlayManager = {
   ...
   */
  cleanup: function() {
    console.log('[MapOverlayManager] 정리 시작');
    
    // 이벤트 리스너 제거
    if (this._mapInstance && this._zoomListener) {
      google.maps.event.removeListener(this._zoomListener);
      this._zoomListener = null;
      console.log('[MapOverlayManager] 줄 리스너 제거 완료');
    }
    
    // 모든 섹션의 오버레이 정리
    Object.keys(this._overlaysBySections).forEach(sectionName => {
      console.log(`[MapOverlayManager] '${sectionName}' 섹션 오버레이 정리 시작`);
      ...
      const collectionPaths = [
        'shops.markers',
        'shops.polygons',
        'landmarks.imageMarkers',
        'hotspots.imageMarkers'
      ];
      
      // 각 커렉션의 오버레이 정리
      collectionPaths.forEach(path => {
        const [category, collection] = path.split('.');
        
        if (section[category]?.[collection]) {
          const delegates = section[category][collection];
          const count = delegates.length;
          
          console.log(`[MapOverlayManager] '${sectionName}' 섹션의 ${category}.${collection} 정리 (${count}개)`);
          
          // 각 델리게이트의 cleanup 메서드 호출
          delegates.forEach(delegate => {
            if (delegate.cleanup) {
              delegate.cleanup(); // 명시적 정리 (내부 참조 해제 포함)
            } else {
              delegate.relieveMap(); // 후방 호환성을 위해 기존 메서드 사용
            }
          });
      ...
      console.log(`[MapOverlayManager] '${sectionName}' 섹션 오버레이 정리 완료`);
    });
    ...
    console.log('[MapOverlayManager] 정리 완료');
  },
   ...
   */
  //차후 개별 item별 ID를 저장해서 중복 방지 로직 추가. existingOverlays.has(item.id) 
