 ...
 * 
 * 3. (LANDMARKS_MARKER): OverlayDelegate
 *    - 내부 오버레이: google.maps.marker.AdvancedMarkerElement
...
const MapOverlayManager = {
  ...
  registerOverlaysByItemlist: async function(sectionName, itemList) {
    ...
    if (this._overlaysBySections[sectionName]) {
      ...
      console.log(`[DEBUG] 섹션 '${sectionName}'의 기존 오버레이 정리 완료`);
    }
    ...
    for (const item of itemList) {
      ...
      switch (category) {
        ...
        if (item.pinCoordinates) {
          ...
          const promise = (async () => {
            console.log(`[DEBUG] 랜드마크 마커 생성 시작: ID=${item.id}`);
            try {
              ...
              if (imageMarker) {
                ...
                const isDuplicate = section.landmarks.imageMarkers.some(
                ...
                if (!isDuplicate) {
                  ...
                  console.log(`[DEBUG] 랜드마크 마커 등록: ${item.id || 'unknown'}`);
                } else {
    ...
    if (pendingPromises.length > 0) {
      console.log(`[DEBUG] ${pendingPromises.length}개의 비동기 랜드마크 마커 생성 대기 중...`);
      await Promise.all(pendingPromises);
      console.log(`[DEBUG] 비동기 랜드마크 마커 생성 완료`);
    }
  ...
  _toggleLayerVisibility: async function(sectionName, layerType) {
    ...
    if (section[sectionCategory] && Array.isArray(section[sectionCategory][collectionName])) {
      ...
      if (overlays.length > 0) {
        ...
        console.log(`[DEBUG] 첫 번째 오버레이 정보: ID=${firstOverlay.itemId || 'unknown'}, 타입=${firstOverlay.type}, 레이어=${firstOverlay.layerType}`);
      }
    } else {
    ...
    const togglePromises = overlays.map(async (overlay, index) => {
      try {
        console.log(`[DEBUG] 오버레이 #${index} 토글 시작: ID=${overlay.itemId || 'unknown'}, 타입=${overlay.type}`);
        
        if (overlay.overlay && overlay.overlay.style) {
          console.log(`[DEBUG] 토글 전 display 상태: ${overlay.overlay.style.display || 'undefined'}`);
        }
        
        await overlay.toggleVisible();
        
        if (overlay.overlay && overlay.overlay.style) {
          console.log(`[DEBUG] 토글 후 display 상태: ${overlay.overlay.style.display || 'undefined'}`);
        }
  ...
  _updateVisibilityForZoom: async function() {
    ...
    Object.entries(LAYER_CONSTANTS).forEach(([layerType, layerInfo]) => {
      ...
      const sectionCategory = layerInfo.section;
      const collectionName = layerInfo.collection;
      const count = section[sectionCategory]?.[collectionName]?.length || 0;
      console.log(`[DEBUG] ${layerType} 레이어의 오버레이 개수: ${count}`);
      
      // 랜드마크 레이어의 경우 모든 항목 로깅
      if (layerType === 'LANDMARKS_MARKER') {
        const landmarks = section.landmarks?.imageMarkers || [];
        landmarks.forEach((marker, idx) => {
          console.log(`[DEBUG] 랜드마크 #${idx}: ID=${marker.itemId}, 이름=${marker.name}, 타입=${marker.type}`);
          
          // 중요: 각 랜드마크 마커의 현재 display 상태 확인 (디버깅용)
          if (marker.overlay && marker.overlay.style) {
            console.log(`[DEBUG] 랜드마크 #${idx} 현재 display 상태: ${marker.overlay.style.display || 'none'}`);
          }
        });
        
        // 랜드마크 마커가 있는데 보이지 않는 문제 해결
        if (landmarks.length > 0 && shouldBeVisible) {
          // 랜드마크 마커를 강제로 표시 (각 마커별로 직접 처리)
          landmarks.forEach(async (marker, idx) => {
            try {
              if (marker.overlay && marker.overlay.style) {
                // 현재 상태 확인
                const currentDisplay = marker.overlay.style.display;
                console.log(`[DEBUG] 랜드마크 #${idx} 직접 가시성 확인: 현재=${currentDisplay}`);
                
                // 보이지 않는 경우에만 강제로 표시
                if (currentDisplay === 'none') {
                  console.log(`[DEBUG] 랜드마크 #${idx} 가시성 강제 변경: none -> block`);
                  marker.overlay.style.display = 'block';
                  
                  // 변경 후 상태 확인
                  console.log(`[DEBUG] 랜드마크 #${idx} 강제 변경 후 상태: ${marker.overlay.style.display}`);
                }
              }
            } catch (e) {
              console.error(`[ERROR] 랜드마크 마커 #${idx} 가시성 강제 변경 실패:`, e);
            }
          });
        }
      }
    ...
    if (togglePromises.length > 0) {
      await Promise.all(togglePromises);
      console.log(`[DEBUG] ${togglePromises.length}개의 레이어 토글 작업 완료`);
    }
    ...
    // 특별 처리: 랜드마크 마커가 있지만 화면에 보이지 않는 문제 디버깅
    if (section.landmarks?.imageMarkers?.length > 0 && section.overlayLayerVisibles.landmarksMarker) {
      console.log('[DEBUG] 랜드마크 마커 가시성 상태 최종 확인');
      section.landmarks.imageMarkers.forEach((marker, idx) => {
        if (marker.overlay && marker.overlay.style) {
          console.log(`[DEBUG] 최종 확인 - 랜드마크 #${idx}, ID=${marker.itemId}, display=${marker.overlay.style.display}, map=${marker.overlay.map ? '있음' : '없음'}`);
          
          // 맵에 제대로 추가되어 있는지 확인
          if (!marker.overlay.map && this._mapInstance) {
            console.log(`[DEBUG] 랜드마크 #${idx}를 맵에 강제 추가`);
