...
// Firebase 모듈 가져오기
// Firebase 관련 유틸리티 함수들을 별도 파일에서 가져옵니다.
import { 
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (count < effectiveLimitCount) {
        ...
        rooms.push({
          ...
          sectionName: roomData.sectionName || 'other'
        });
 ...
 */
export const setupChatListener = (roomId, communityDBManager, options = {}) => {
  ...
  try {
    ...
    const unsubscribe = onSnapshot(messagesQuery, (querySnapshot) => {
      ...
      const messages = querySnapshot.docs.map(doc => ({
        ...
        isSystemMessage: doc.data().isSystemMessage || false,
      ...
      // 구독자에게 단일 알림 전송
      communityDBManager.notifySubscribers('messagesUpdated', { roomId, messages });
    }, (error) => {
      logError(`리스너 오류: ${roomId}`, error);
      communityDBManager.notifySubscribers('error', { roomId, error: error.message });
    });
