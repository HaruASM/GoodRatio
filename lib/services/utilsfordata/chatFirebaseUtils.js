...
export async function checkRoomPermission(roomId, userId, checkReadOnly = false) {
  ...
  // 채팅방 접근 권한 확인
  if (!roomData.isPublic && !roomData.members.includes(userId)) {
    throw new Error('permission_denied');
  }
  
  // 읽기 전용 채팅방 검사
  if (checkReadOnly && roomData.readOnly && !roomData.admins?.includes(userId)) {
...
/**
 * 캅슈화된 getDocs 함수 (캐싱 기능 추가 및 유효성 검사)
