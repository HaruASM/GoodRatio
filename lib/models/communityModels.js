...
export const dispatchCommunityEvent = (eventType, detail, target) => {
  if (!target || typeof target.dispatchEvent !== 'function') {
    ...
    // 오류 발생 시 document.body에 오류 이벤트 디스패치
    try {
      if (document && document.body) {
        const errorEvent = new CustomEvent(CommunityEventTypes.ERROR, {
          detail: { error: `이벤트 대상 누락: ${eventType}` }
        });
        document.body.dispatchEvent(errorEvent);
      }
    } catch (e) {
      console.error(`[CommunityModels] 대체 오류 처리 실패:`, e);
    }
    return;
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[CommunityModels] 이벤트 발행: ${eventType} on DOM element`, detail);
  }
};