...
const ModuleManager = {
  ...
  initialize: function() {
    ...
    ModuleManager.register('sectionDBManager', () => {
      console.log('[ModuleManager] SectionDBManager 모듈 지연 초기화 실행');
      
      // 동적으로 SectionDBManager 가져오기
      const SectionDBManager = require('../lib/components/data/SectionDBManager').default;
      console.log('[ModuleManager] SectionDBManager 모듈 로드 완료');
    ...
    ModuleManager.register('mapViewMarking', () => {
      ...
      // 맵 관련 상태를 관리하는 모듈 객체
      const mapViewMarkingModule = {
        mapInstance: null,
        
        initialize: function(mapInstance) {
          if (mapInstance) {
            this.mapInstance = mapInstance;
            console.log('[MapViewMarking] 맵 인스턴스 설정 완료');
          }
          return this;
        },
        ...
        // 일시 중단 시 호출 (페이지 전환 시)
        suspend: function() {
          console.log('[MapViewMarking] 모듈 일시 중단 (맵 인스턴스 유지)');
          // 맵 인스턴스는 유지하고 필요한 리소스만 해제
          return true;
        },
        
        // 정리 시 호출 (앱 종료 시)
        cleanup: function() {
          console.log('[MapViewMarking] 모듈 정리 (맵 인스턴스 해제)');
