...
// travelCommunity컴포넌트 UI - communityAPI 커뮤니티의 채널/채팅/메세지 데이터 관리 - realtimeChatUtilsFB는 유틸리티 컴포넌트. fireBase서버 기능담당
// travelCommunity 컴포넌트가 메인 로직에서 사용되고, travelCommunity 내부에서 communityAPI와 realtimeChatUtilsFB를 사용한다. 
// travelCommunity컴포넌트 <-> communityAPI.js <-> realtimeChatUtilsFB.js
...
export const sendMessage = async (roomId, messageData) => {
  // options and target removed, validateEventTarget removed
  try {
    ...
    // dispatchCommunityEvent for messageSent removed

    log(`메시지 전송 성공: ${roomId}, ${docRef.id}`);
    return docRef.id;
  } catch (error) {
    // dispatchCommunityEvent for error removed
...
/**
 * 실시간 채팅 리스너 설정 (개선된 버전 - 콜백 함수 방식으로 변경)
 * @param {string} roomId - 채팅방 ID
 * @param {Function} callback - 메시지 변경 시 호출할 콜백 함수
 * @param {Object} options - 추가 옵션
 * @returns {Function} 리스너 해제 함수
 */
export const listenForMessages = async (roomId, callback, options = {}) => {
  try {
    if (!roomId || typeof callback !== 'function') {
      log('유효하지 않은 매개변수: roomId 또는 callback 함수가 필요합니다.');
      return () => {}; // 빈 함수 반환
    }
    ...
    logError(`유효하지 않은 매개변수: roomId 또는 callback 함수가 필요합니다.`, error);
    return () => {};
  }
};
...
export const manageChatRoomMember = async (roomId, userId, join = true, options = {}) => {
  try {
    ...
    // 콜백 함수 호출 (선택적)
    if (notifySubscribers && callback && typeof callback === 'function') {
      callback({
        roomId,
        userId,
 ...
 * 타이핑 상태 리스너 설정
 * @param {string} roomId - 채팅방 ID
 * @param {Function} callback - 타이핑 상태 변경 콜백
 * @returns {Function} 리스너 해제 함수
...
export const markMessagesAsRead = async (roomId, messageIds, userId, options = {}) => {
  try {
    ...
    // 구독자에게 알림
    if (options.notifySubscribers !== false && options.callback) {
      options.callback({
        roomId,
        userId,
