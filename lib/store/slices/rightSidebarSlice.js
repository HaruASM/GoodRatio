...
import { confirmImageOrder } from './imageGallerySlice';
...
const rightSidebarSlice = createSlice({
  ...
  reducers: {
    ...
    // 이미지 순서 업데이트 리듀서 추가
    updateImagesOrder: (state, action) => {
      const { mainImage, subImages } = action.payload;
      
      if (state.formData) {
        // 메인 이미지 업데이트
        if (mainImage) {
          state.formData.mainImage = mainImage;
        }
        
        // 서브 이미지 업데이트
        if (Array.isArray(subImages)) {
          state.formData.subImages = subImages;
        }
      }
    },
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      // 이미지 갤러리의 confirmImageOrder 액션 처리
      .addCase(confirmImageOrder, (state, action) => {
        const { mainImage, subImages, hasMainImage } = action.payload || {};
        
        // 유효성 검사
        if (!state.editNewitemdataSet) return;
        
        // hasMainImage 값이 false인 경우, mainImage는 빈 문자열로 설정
        if (hasMainImage === false) {
          state.editNewitemdataSet.mainImage = '';
          state.formData.mainImage = '';
        } 
        // mainImage가 존재하고 hasMainImage가 true인 경우 업데이트
        else if (typeof mainImage === 'string' && hasMainImage) {
          state.editNewitemdataSet.mainImage = mainImage;
          state.formData.mainImage = mainImage;
        }
        
        // subImages 업데이트 (유효성 검사 후)
        if (Array.isArray(subImages)) {
          state.editNewitemdataSet.subImages = subImages;
          state.formData.subImages = subImages;
        }
        
        // 필드 변경 추적
        state.modifiedFields.mainImage = true;
        state.modifiedFields.subImages = true;
        state.hasChanges = true;
        
        // 변경된 필드 리스트에 추가
        if (!state.changedFieldsList.includes('mainImage')) {
          state.changedFieldsList.push('mainImage');
        }
