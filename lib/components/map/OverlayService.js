...
class OverlayDelegate {
  ...
  relieveMap() {
    if (this.overlay) {
      ...
      // 줌 변경 이벤트 리스너 제거 (이미지 마커에서 추가된 경우 삭제시킴)
      if (this.overlay._zoomChangeListener) {
        google.maps.event.removeListener(this.overlay._zoomChangeListener);
        this.overlay._zoomChangeListener = null;
      }
    }
  }
  
  /**
   * 오버레이 완전 정리 - 맵에서 제거하고 내부 참조 해제
   */
  cleanup() {
    // 맵에서 제거
    this.relieveMap();
    
    // AdvancedMarkerElement의 경우 DOM 참조 제거
    if (this.overlay instanceof google.maps.marker.AdvancedMarkerElement) {
      if (this.overlay.content) {
        this.overlay.content = null; // DOM 참조 제거
      }
    }
...
const OverlayService = {
  ...
  createImageMarker: async function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    ...
    try {
      ...
      markerContainer.style.width = `${_baseWidth}px`;
      markerContainer.style.height = `${_baseHeight}px`;
      ...
      if (options) {
        ...
        logoContainer.appendChild(  createLogoElement('번화가', '#FF5722') );
      ...
      const baseZoom = 17;
      const zoomforLogo = 16;
      
      // 줌 변경 이벤트에 따른 마커 크기 조절
      const zoomChangeListener = google.maps.event.addListener(
        mapInstance,
        'zoom_changed',
        () => {
          const currentZoom = mapInstance.getZoom();
          console.log(`[DEBUG] 이미지 마커 줌 레벨 변경: ${currentZoom}`);
          
          // 줌 레벨에 따른 크기 조정 계산
          // 줌이 커질수록(확대) 마커도 커지도록 계산
          // baseZoom 이상 확대에서는 baseZoom유지 
          if(currentZoom < baseZoom) { //크기가 축소되는 zoom에서 작동

            
            // 최소/최대 크기 제한
            const minSize = 60;  // 최소 크기
            const maxSize = _baseWidth; // 최대 크기
            // 새 크기 계산
            
            const imageOvelayScale = 1.25;
            
            const zoomDiff = currentZoom - baseZoom;
            const scaleFactor = zoomDiff >= 0 ? 
                              1 : //Math.pow(imageOvelayScale, zoomDiff) : // 줌이 크면 확대 (1.2의 양수 지수) // currentZoom >= baseZoom  일때의 케이스라서 1 스케일로 지정. 이 로직이 동작하는 분기에서는 이 부분이 필요없긴 하다. 
                              1 / Math.pow(imageOvelayScale, Math.abs(zoomDiff)); 
            
            const newWidth = Math.round(_baseWidth * scaleFactor);
            const newHeight = Math.round(_baseHeight * scaleFactor);
            ...
            // 마커 컨테이너 크기 변경
            markerContainer.style.width = `${limitedWidth}px`;
            markerContainer.style.height = `${limitedHeight}px`;
            
            // 로고 컨테이너 스케일 조정
            const logoContainer = markerContainer.querySelector('.logo-container');
            if (logoContainer) {
              // 줌 레벨이 16 이상일 때만 로고 표시
              if (currentZoom >= zoomforLogo) {
                const logoScaleFactor = Math.max(1, Math.min(2, scaleFactor));
                logoContainer.style.transform = `scale(${logoScaleFactor})`;
                logoContainer.style.transformOrigin = 'top left';
                
                // 로고 위치 조정
                const newPadding = Math.max(8, Math.min(16, Math.round(8 * scaleFactor)));
                logoContainer.style.top = `${newPadding}px`;
                logoContainer.style.left = `${newPadding}px`;
          ...
          } else { // 크기조정이 필요없는 zoom 레벨에서 노말 사이즈 . //baseZoom >= currentZoom // scaleFactor를 규정하는 연상하는 부분에서 baseZoom >= currentZoom일때 1로 지정하면 이 분기가 필요없긴 하다. 
      ...
      // 마커 객체에 리스너 참조 저장 (나중에 정리할 수 있도록)
      imageMarker._zoomChangeListener = zoomChangeListener;
  ...
  createToggleablePolygon: function(path, options = {}) {
    ...
    polygon.addListener('mouseover', function(e) {
      ...
      if (!itemId || !sectionName) return;
      
      // ModuleManager를 통해 SectionDBManager 로드
      import('../../moduleManager').then(async (module) => {
        try {
          const ModuleManager = module.default;
          const sectionDBManager = await ModuleManager.loadGlobalModule('sectionDBManager');
          
          if (!sectionDBManager) {
            console.error('[OverlayService] SectionDBManager 모듈을 로드할 수 없습니다.');
            return;
          }
          
          // 아이템 데이터 가져오기
          const item = await sectionDBManager.getItemByIDandSectionName(itemId, sectionName);
          ...
          console.error('[OverlayService] 스트리트뷰 정보창 생성 중 오류:', error);
        }
      }).catch(error => {
        console.error('[OverlayService] ModuleManager 로드 오류:', error);
      });
    }.bind(this));
   ...
   */
  createToggleableImageMarker: async function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    try {
