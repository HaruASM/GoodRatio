 ...
 * 
 * @param {Object} itemdata - 상점 데이터
 * @param {Object} currentFormData - 현재 폼 데이터
 * @returns {Object} 업데이트된 폼 데이터
 */
export const updateFormDataFromShop = (itemdata, currentFormData = {}) => {
  // 빈 값 처리: itemdata가 없거나 정의되지 않은 경우 protoServerDataset 사용
  // 데이터는 직접 사용 (itemdata가 없으면 protoServerDataset 사용)
  const data = itemdata || protoServerDataset;
  ...
  // iconDesign 처리 함수 추가 - 0이 아닌 경우 "등록됨"으로 표시
  const formatIconDesign = (iconDesign) => {
    console.log("formatIconDesign 호출됨 ");
    if (iconDesign && iconDesign !== 0) {
  ...
  // streetView 처리 함수 추가 - panoid가 있는 경우 "등록됨"으로 표시
  const formatStreetView = (streetView) => {
    // RightSidebar.js와 동일한 로직 사용: streetView 객체와 panoid 값이 있을 때만 "등록됨" 반환
    console.log("formatStreetView 호출됨");
    
    // streetView가 undefined나 null인 경우에 대한 안전한 체크
    if (!streetView) {
      console.log("streetView 없음");
      return "";
    }
    
    console.log("streetView 객체:", streetView);
    console.log("streetView.panoid:", streetView.panoid);
    
    const isValidStreetView = streetView && streetView.panoid && streetView.panoid !== "";
    console.log("유효한 streetView?", isValidStreetView);
    
    return isValidStreetView ? "등록됨" : "";
  };
  ...
  return {
    ...
    iconDesign: formatIconDesign(data.iconDesign),
    streetView: formatStreetView(data.streetView)
  };
};
 ...
 * @returns {boolean} 변경 사항 존재 여부
 */
export const compareitemdata = checkDataIsChanged;
 ...
 */
export const getDefaultitemdata = () => {
