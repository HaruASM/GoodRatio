...
const OverlayService = {
   ...
   * @returns {google.maps.marker.AdvancedMarkerElement|null} 생성된 마커 또는 null
   */
  createImageMarker: function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    if (!mapInstance) {
    ...
    console.log('[OverlayService] 원본 좌표:', coordinates);
    console.log('[OverlayService] 맵 중심 좌표 사용:', mapCenterCoords);
    
    // 맵 중심 좌표 사용
    coordinates = mapCenterCoords;
    ...
    try {
      // 테스트용 샘플 이미지 직접 사용 (publicId 무시)
      const imageUrl = 'https://images.unsplash.com/photo-1514924013411-cbf25faa35bb?q=80&w=600&auto=format&fit=crop';
      console.log('[OverlayService] 테스트 이미지 사용:', imageUrl);
      ...
      const imageMarker = new window.google.maps.marker.AdvancedMarkerElement({
        ...
        title: title || '',
        map: mapInstance  // 처음에는 맵에 추가하지 않음
      });
      ...
      const zoomChangeListener = google.maps.event.addListener(
        ...
        () => {
          ...
          // 로고 요소 찾기 및 크기 조정 - 모든 로고 박스와 텍스트 선택
          const logoBoxes = markerContainer.querySelectorAll('.logo-box');
          logoBoxes.forEach(logoBox => {
            // 로고 박스 패딩 조정
            const newLogoPadding = Math.max(2, Math.min(6, Math.round(2 * scaleFactor)));
            logoBox.style.padding = `${newLogoPadding}px ${newLogoPadding * 2}px`;
            
            // 높이도 스케일링
            const baseLogoHeight = 16;
            const newLogoHeight = Math.max(14, Math.min(32, Math.round(baseLogoHeight * scaleFactor)));
            logoBox.style.height = `${newLogoHeight}px`;
            
            // 로고 텍스트 찾기
            const logoText = logoBox.querySelector('.logo-text');
            if (logoText) {
              const baseLogoFontSize = 10;
              const newLogoFontSize = Math.max(8, Math.min(16, Math.round(baseLogoFontSize * scaleFactor)));
              logoText.style.fontSize = `${newLogoFontSize}px`;
            }
          });
          
          // 캡션 요소 찾기 및 폰트 크기 설정
          const captionElement = markerContainer.querySelector('.caption-text');
          if (captionElement) {
   ...
   */
  createToggleableImageMarker: function(mapInstance, coordinates, publicId, title, baseWidth = 120, baseHeight = 80, options = {}) {
    // 기본 이미지 마커 생성
    const imageMarker = this.createImageMarker(mapInstance, coordinates, publicId, title, baseWidth, baseHeight, options);
  ...
  createOverlayOfLandmarkMarker: function(mapInstance, item, sectionName) {
    ...
    // 이미지 ID 준비 (pictureIcon 또는 imageUrl 필드 사용)
    const publicId = item.publicId || item.pictureIcon || item.imageUrl || item.id;
    const title = item.name || item.itemName || '';
    ...
    const imageMarkerDelegate = this.createToggleableImageMarker(
      ...
      80,  // 기본 높이
      { 
        logoType: item.category === 'tourist' ? 'tourist' : 'downtown' 
      }
    );
  ...
  createOverlayOfHotspotMarker: function(mapInstance, item, sectionName) {
    ...
    try {
      ...
      if (item.iconDesign) {
        imageMarkerDelegate.iconDesign = item.iconDesign;
      }
