/**
 * AI 개발 가이드: 이미지 헬퍼 유틸리티 시스템
 * -----------------------------------------------------
 * 주요 기능 및 설계 원칙:
 * 1. 이미지 URL 생성: 템플릿 기반 이미지 URL 생성으로 일관된 이미지 형식 제공
 * 2. 배치 프리캐싱: 구글 이미지 참조를 효율적으로 처리하기 위한 배치 처리 메커니즘
 * 3. Next.js 통합: Image 컴포넌트와 원활히 작동하기 위한 props 생성 헬퍼
 * 4. 오류 내성: 이미지 누락, 로딩 실패 등의 상황에 대한 폴백 메커니즘 구현
 * 
 * 사용 시 주의사항:
 * 1. publicId 경로 처리: 물리 경로(map-Images/)와 논리 경로 간 차이 주의
 * 2. 중복 확인: 이미지 존재 여부 확인 시 getFullPublicId 함수 사용 필수
 * 3. 비동기 처리: 이미지 프리캐싱은 비동기적으로 이루어지므로 적절한 에러 핸들링 필요
 * 4. 템플릿 타입: IMAGE_TEMPLATES 객체에 정의된 템플릿 타입만 사용
 * 
 * 핵심 함수:
 * - getTemplatePhotoUrl: 템플릿 기반 이미지 URL 생성
 * - batchPreCacheImagesForGoggleReferece: 구글 이미지 배치 프리캐싱
 * - createNextImageProps: Next.js Image 컴포넌트용 속성 생성
 * -----------------------------------------------------
 */
...
export async function createNextImageProps(publicId, template = IMAGE_TEMPLATES.ORIGINAL, options = {}) {
  ...
  //FIXME  cloudinary에서 템플릿으로 이미지를 로딩해오는데 이런 설정이 왜 필요하지. 
  const {
    width = 400,
    height = 300,
    objectFit = "cover",
...
/**
 * 일반 DOM img 요소를 위한 이미지 URL 생성 함수
 * 이 함수는 API를 호출하여 Cloudinary URL을 가져와 이미지 로딩에 사용합니다
 * 
 * @param {string} publicId - 이미지 Public ID
 * @param {string} template - 템플릿 타입 (IMAGE_TEMPLATES 값 중 하나)
 * @param {function} callback - 이미지 URL이 준비되면 호출될 콜백 함수
 * @param {Object} options - 추가 옵션
 * @returns {Promise<Object>} 이미지 속성 객체
 */
export async function createDomImgProps(publicId, template = IMAGE_TEMPLATES.ORIGINAL, callback = null, options = {}) {
  if (!publicId || typeof publicId !== 'string' || publicId.trim() === '') {
    console.error('유효한 publicId가 필요합니다');
    return null;
  }
  ...
  try {
    // API 호출하여 실제 Cloudinary URL 가져오기
    const apiUrl = `/api/place-photo?public_id=${publicId}&template=${template}`;
    const response = await fetch(apiUrl);
    ...
    console.log('DOM 이미지 속성 생성 오류:', error.message);
    if (onError) onError();
    return null;
  }
} 