...
/**
 * 커뮤니티 커스텀 이벤트 타입 정의
 * @description 모듈 간 통신을 위한 커스텀 이벤트 이름과 데이터 구조를 정의합니다.
 */
...
export const CommunityEventTypes = {
  CHAT_ROOMS_LOADED: 'community:chatRoomsLoaded', // Added
  MESSAGES_UPDATED: 'community:messagesUpdated',
  TYPING_STATUS: 'community:typingStatus',
  MESSAGE_SENT: 'community:messageSent',
  MESSAGE_DELETED: 'community:messageDeleted',
  ROOM_MEMBER_CHANGED: 'community:roomMemberChanged',
  ERROR: 'community:error',
  CHAT_INITIALIZED: 'community:chatInitialized', // Kept from original
};

/**
 * 채팅방 목록 및 초기 데이터 로드 완료 이벤트 데이터
 * @typedef {Object} ChatRoomsLoadedEventDetail
 * @property {ChatRoom[]} rooms - 로드된 채팅방 목록
 * @property {string|null} roomId - 초기에 선택된 채팅방 ID (없을 경우 null)
 * @property {ChatMessage[]} messages - 초기 선택된 채팅방의 메시지 목록 (roomId가 있을 경우)
 */

/**
 * 메시지 목록 업데이트 이벤트 데이터
 * @typedef {Object} MessagesUpdatedEvent
 * @property {string} roomId - 채팅방 ID
 * @property {ChatMessage[]} messages - 업데이트된 메시지 배열
 */

/**
 * 타이핑 상태 변경 이벤트 데이터
 * @typedef {Object} TypingStatusEvent
 * @property {string} roomId - 채팅방 ID
 * @property {Object.<string, boolean>} typingUsers - 타이핑 중인 사용자 ID와 상태
 */
 ...
 * @property {ChatMessage} messageData - 전송된 메시지 데이터
 */
 ...
 * @property {string} roomId - 채팅방 ID
 * @property {string} messageId - 삭제된 메시지 ID
 */

/**
 * 채팅방 멤버 변경 이벤트 데이터
 * @typedef {Object} RoomMemberChangedEvent
 * @property {string} roomId - 채팅방 ID
 * @property {string} userId - 변경된 사용자 ID
 * @property {'join' | 'leave'} action - 멤버 변경 작업
 */
...
/**
 * 채팅 초기화 이벤트 데이터
 * @typedef {Object} ChatInitializedEvent
 * @property {Object} userInfo - 사용자 정보
 * @property {ChatRoom[]} rooms - 채팅방 목록
 * @property {ChatMessage[]} messages - 메시지 목록
 * @property {string|null} roomId - 현재 채팅방 ID
 */

/**
 * 커스텀 이벤트 디스패치 헬퍼 함수
 * @param {string} eventType - CommunityEventTypes의 이벤트 타입 (예: community:chatRoomsLoaded)
 * @param {Object} detail - 이벤트 데이터 (예: { rooms: ChatRoom[] })
 * @param {EventTarget} target - 이벤트를 디스패치할 대상 (.travelCommunity-container)
 */
export const dispatchCommunityEvent = (eventType, detail, target) => {
  if (!target || typeof target.dispatchEvent !== 'function') {
    console.warn(`[CommunityModels] 유효하지 않은 이벤트 대상: ${eventType}`, new Error().stack);
    // CommunityEventTypes.ERROR를 사용하여 오류 이벤트 발행
    // 이 함수가 재귀적으로 호출될 수 있으므로, document.body는 일반적으로 안전한 fallback임
    // 단, CommunityEventTypes.ERROR가 정상적으로 정의되어 있어야 함 (현재 'community:error'로 정의됨)
    const errorEvent = new CustomEvent(CommunityEventTypes.ERROR, { detail: { error: `이벤트 대상 누락: ${eventType}` } });
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[CommunityModels] 이벤트 발행: ${eventType}`, detail);
  }
};