...
class CommunityDBManager {
  ...
  async getChatMessages(roomId, messageLimit = 50) {
    try {
      ...
      console.error(`[CommunityDBManager] 채팅방 ${roomId} 메시지 가져오기 오류:`, {
        message: error.message,
        code: error.code,
        details: error.details,
        stack: error.stack,
  ...
  /**
   * 모든 구독자에게 데이터 변경 알림
   * @param {string} eventType - 이벤트 타입 ('messages', 'messagesRead', 'messageEdited', 'messageDeleted', 'typingStatus' 등)
   * @param {Object} data - 이벤트 데이터
   */
  notifySubscribers(eventType, data) {
    // 이전 버전과의 호환성을 위해 roomId와 messages를 직접 전달하는 경우 처리
    if (typeof data === 'object' && Array.isArray(data) && typeof eventType === 'string' && !eventType.includes(':')) {
      // 이전 호출 방식: notifySubscribers(roomId, messages)
      const roomId = eventType;
      const messages = data;
    ...
    // 새로운 호출 방식: notifySubscribers(eventType, data)
    this.subscribers.forEach(subscriber => {
      try {
        // 구독자가 새 형태의 호출을 지원하는지 확인
        if (subscriber.length >= 2) {
          subscriber(eventType, data);
        } else if (eventType === 'messages' && data.roomId && data.messages) {
          // 이전 형태의 구독자지만 messages 이벤트인 경우 변환해서 호출
          subscriber(data.roomId, data.messages);
        }
      } catch (error) {
        console.error(`구독자 알림 오류:`, error);
      }
