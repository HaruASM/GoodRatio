...
// 커스텀 이벤트 발행 헬퍼 함수
const dispatchCustomEvent = (eventName, detail, target = window) => {
  // 이전 방식과의 호환성을 위해 이벤트 이름에 네임스페이스 추가
  const eventType = `community:${eventName}`;
  const event = new CustomEvent(eventType, { detail });
  
  // target이 유효한지 확인 (강화된 유효성 검사)
  if (!target || typeof target.dispatchEvent !== 'function') {
    console.warn(`[CommunityDBManager] 유효하지 않은 이벤트 대상: ${eventType}, window로 대체`);
    window.dispatchEvent(event); // 기본적으로 window에 디스패치
  } else if (target instanceof Element && target.classList.contains('travelCommunity-container')) {
    // .travelCommunity-container에 이벤트 디스패치
    target.dispatchEvent(event);
  } else if (target instanceof Element && document.querySelector('.travelCommunity-container')) {
    // target이 Element이지만 .travelCommunity-container가 아닌 경우, 실제 컨테이너로 대체
    document.querySelector('.travelCommunity-container').dispatchEvent(event);
  } else {
    // 그 외의 경우 제공된 target에 디스패치
    target.dispatchEvent(event);
  }
  
  console.log(`[CommunityDBManager] 이벤트 발행: ${eventType} on ${target === window ? 'window' : 'DOM element'}`, detail);
};
...
class CommunityDBManager {
   ...
   * @param {Object} options - 추가 옵션
   * @param {EventTarget} [options.target=window] - 이벤트를 디스패치할 대상
  ...
  async sendMessage(roomId, messageData, options = {}) {
    try {
      ...
      // 일관된 오류 처리
      this.notifySubscribers('error', { roomId, error: error.message }, options.target || window);
      throw error;
    }
  }
   ...
   */
  setupChatListener(roomId, callback, options = {}) {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(messagesQuery, (snapshot) => {
        ...
        this.notifySubscribers('error', { roomId, error: error.message }, target);
      });
      ...
      if (options.target) {
        ...
        this.notifySubscribers('error', { roomId, error: error.message }, window);
      }
      return () => {};
    }
  }
   ...
   * @param {EventTarget} [target=window] - 이벤트를 디스패치할 대상
   */
  notifySubscribers(eventType, data, target = window) {
    // target 유효성 검사 추가
    if (target && !(target instanceof EventTarget)) {
      console.warn(`[CommunityDBManager] 유효하지 않은 이벤트 대상 타입: ${typeof target}, window로 대체`);
      target = window;
    }
  ...
  async editMessage(roomId, messageId, newMessage, userId, options = {}) {
    try {
      ...
      clearCache('messages', roomId);
      this.notifySubscribers('messageEdited', { roomId, messageId, newMessage, userId, timestamp: new Date() }, target);
  ...
  async deleteMessage(roomId, messageId, userId, options = {}) {
    try {
      ...
      clearCache('messages', roomId);
      clearCache('chatRooms');
      this.notifySubscribers('messageDeleted', { roomId, messageId, userId, timestamp: new Date() }, target);
