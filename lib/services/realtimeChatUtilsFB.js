...
// 커스텀 이벤트 발행 헬퍼 함수
const dispatchCustomEvent = (eventName, detail, target) => {
  if (!target || typeof target.dispatchEvent !== 'function') {
    console.warn(`[realtimeChatUtilsFB] 유효하지 않은 이벤트 대상: community:${eventName}`);
    return;
  }
  const event = new CustomEvent(`community:${eventName}`, { detail });
  target.dispatchEvent(event);
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[realtimeChatUtilsFB] 이벤트 발행: community:${eventName} on DOM element`, detail);
  }
};
...
export const sendMessage = async (roomId, messageData, options = {}) => {
  try {
    ...
    if (!target || typeof target.dispatchEvent !== 'function') {
      throw new Error('유효한 이벤트 대상이 지정되지 않았습니다.');
    }
    ...
    if (options.target && typeof options.target.dispatchEvent === 'function') {
      dispatchCustomEvent('error', { roomId, error: error.message }, options.target);
    }
 ...
 */
export const setupChatListener = (roomId, communityDBManager, options = {}, target) => {
