...
const dispatchCustomEvent = (eventName, detail, target = window) => {
  ...
  if (!target || typeof target.dispatchEvent !== 'function') {
    console.warn(`[CommunityDBManager] 유효하지 않은 이벤트 대상: ${eventType}`);
...
class CommunityDBManager {
  ...
  async sendMessage(roomId, messageData, options = {}) {
    try {
      ...
      // 메시지 목록 업데이트 이벤트도 발행
      this.notifySubscribers('messagesUpdated', { 
        roomId, 
        messages: await this.getChatMessages(roomId) 
      }, target);
   ...
   */
  setupChatListener(roomId, callback, options = {}, target = window) {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(messagesQuery, (snapshot) => { 
        ...
        // target 매개변수를 전달하여 특정 DOM 요소에 이벤트 디스패치
  ...
  notifySubscribers(eventType, data, target = window) {
    // 이전 방식과의 호환성을 위해 dispatchCustomEvent 사용
    dispatchCustomEvent(eventType, data, target);
    
    // 새로운 표준화된 이벤트 시스템을 위해 CommunityEventTypes와 dispatchCommunityEvent 사용
    // 이벤트 타입에 따라 적절한 CommunityEventTypes 상수 매핑
    let communityEventType;
    switch(eventType) {
      case 'messagesUpdated':
        communityEventType = CommunityEventTypes.MESSAGES_UPDATED;
        break;
      case 'typingStatus':
        communityEventType = CommunityEventTypes.TYPING_STATUS;
        break;
      case 'messageSent':
        communityEventType = CommunityEventTypes.MESSAGE_SENT;
        break;
      case 'messageDeleted':
        communityEventType = CommunityEventTypes.MESSAGE_DELETED;
        break;
      case 'roomMemberChanged':
        communityEventType = CommunityEventTypes.ROOM_MEMBER_CHANGED;
        break;
      case 'error':
        communityEventType = CommunityEventTypes.ERROR;
        break;
      default:
        // 정의되지 않은 이벤트 타입은 기존 방식으로만 발행
        return;
    }
    
    // 표준화된 이벤트 디스패치
    dispatchCommunityEvent(communityEventType, data, target);
  }
