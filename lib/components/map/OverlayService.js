...
const OverlayService = {
  ...
  createToggleableImageMarker: async function(mapInstance, coordinates, publicId, title, baseWidth, baseHeight, options = {}) {
    try {
      console.log(`[DEBUG][createToggleableImageMarker] 호출: publicId=${publicId}, 좌표=${JSON.stringify(coordinates)}`);
      
      // 맵 인스턴스 확인
      if (!mapInstance) {
        console.error('[ERROR][createToggleableImageMarker] 맵 인스턴스가 제공되지 않았습니다');
        return null;
      }
      ...
      console.log(`[DEBUG][createToggleableImageMarker] 이미지 마커 생성 성공, 델리게이트 래핑 중`);
      ...
      // 맵에 추가 (맵 인스턴스가 있는 경우)
      if (mapInstance) {
        console.log(`[DEBUG][createToggleableImageMarker] 맵에 마커 추가 시도`);
        imageMarkerDelegate.setMap(mapInstance);
        
        // 맵에 실제로 추가되었는지 확인 (디버깅 목적)
        if (imageMarker.map) {
          console.log(`[DEBUG][createToggleableImageMarker] 맵 설정 성공: ${!!imageMarker.map}`);
        } else {
          ...
          try {
            ...
            console.log(`[DEBUG][createToggleableImageMarker] 맵 설정 재시도 완료`);
          } catch (e) {
      ...
      console.log(`[DEBUG][createToggleableImageMarker] 완료 - ID=${imageMarkerDelegate.itemId}, 타입=${imageMarkerDelegate.type}`);
