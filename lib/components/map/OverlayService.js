...
const OverlayService = {
  ...
  createMarker: function(coordinates, title = "", options = {}) {
    ...
    try {
      ...
      // AdvancedMarkerElement 생성 - 맵 인스턴스는 나중에 설정하도록 함
   ...
   * @returns {google.maps.marker.AdvancedMarkerElement|null} 생성된 마커 또는 null
   */
  createImageMarker: function(mapInstance, lat, lng, imageUrl, options = {}) {
    if (!mapInstance) {
    ...
    if (!lat || !lng) {
      console.error('[OverlayService] 이미지 마커 생성 실패: 위도 또는 경도가 제공되지 않았습니다');
      return null;
    }
  ...
  isValidCoords: function(coordinates) {
    ...
    try {
      ...
      console.error('[OverlayService] 좌표 유효성 검사 중 오류 발생:', error);
      return false;
    }
  },
};
